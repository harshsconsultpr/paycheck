...
$ent_url = \Drupal::config('miniorange_saml.settings')->get('miniorange_saml_entity_id');
$base_site_url = isset($b_url)? $b_url : $base_url;	
$entity_id_url = isset($ent_url)? $ent_url : $base_url;	
$site_url = substr($base_site_url, 0, strpos($base_site_url, '/modules'));   
$module_path = drupal_get_path('module', 'miniorange_saml');
$entity_id = $entity_id_url;
$acs_url = $base_site_url . '/samlassertion'; 
$certificate = file_get_contents(\Drupal::root() . DIRECTORY_SEPARATOR . $module_path . DIRECTORY_SEPARATOR . 'resources' . DIRECTORY_SEPARATOR . 'sp-certificate.crt' );
...
<md:EntityDescriptor xmlns:md="urn:oasis:names:tc:SAML:2.0:metadata" validUntil="2020-10-28T23:59:59Z" cacheDuration="PT1446808792S" entityID="' . $entity_id . '">
  <md:SPSSODescriptor AuthnRequestsSigned="true" WantAssertionsSigned="true" protocolSupportEnumeration="urn:oasis:names:tc:SAML:2.0:protocol">
	...
	<md:SingleLogoutService Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST" Location="' . $base_site_url . '/user/logout"/>
    <md:SingleLogoutService Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect" Location="' . $base_site_url . '/user/logout"/>
