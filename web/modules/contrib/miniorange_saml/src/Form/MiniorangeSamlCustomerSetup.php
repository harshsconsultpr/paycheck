...
class MiniorangeSamlCustomerSetup extends FormBase {
  ...
  public function buildForm(array $form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    if ($current_status == 'PLUGIN_CONFIGURATION' && \Drupal::config('miniorange_saml.settings')->get('miniorange_saml_license_key') == NULL) {
...
function miniorange_saml_validate_license_submit(&$form, $form_state)
   {
        ...
        {
            $key = \Drupal::config('miniorange_saml.settings')->get('miniorange_saml_customer_admin_token');
            \Drupal::configFactory()->getEditable('miniorange_saml.settings')->set('miniorange_saml_license_key', AESEncryption::encrypt_data($code, $key))->save();
            \Drupal::configFactory()->getEditable('miniorange_saml.settings')->set('t_site_status', AESEncryption::encrypt_data("false", $key))->save();

            \Drupal::configFactory()->getEditable('miniorange_saml.settings')->set('site_ck_l', AESEncryption::encrypt_data("true", $key))-> save();
            drupal_set_message(t('Your license is verified. You can now setup the plugin.'));

            /**
             *	Encrypting installation and domain paths and storing to database
             */
            ...
            \Drupal::configFactory()->getEditable('miniorange_saml.settings')->set('minorange_saml_customer_admin_fraud_check', AESEncryption::encrypt_data($fraud_check, $key))-> save();
    ...
    */
    ...
    function miniorange_saml_remove_account(&$form, $form_state)
    {
        if (isset($_POST['value_check']) && $_POST['value_check'] == 'True')
        {
            $current_status = 'CUSTOMER_SETUP';
            $clear_value = '';
            \Drupal::configFactory()->getEditable('miniorange_saml.settings')->set('miniorange_saml_customer_admin_email', $clear_value)->save();
            \Drupal::configFactory()->getEditable('miniorange_saml.settings')->set('miniorange_saml_customer_setup_phone', $clear_value)->save();
            \Drupal::configFactory()->getEditable('miniorange_saml.settings')->set('miniorange_saml_customer_api_key', $clear_value)->save();
            \Drupal::configFactory()->getEditable('miniorange_saml.settings')->set('miniorange_saml_customer_admin_token', $clear_value)->save();
            \Drupal::configFactory()->getEditable('miniorange_saml.settings')->set('miniorange_saml_license_key', $clear_value)->save();
            \Drupal::configFactory()->getEditable('miniorange_saml.settings')->set('miniorange_saml_customer_setup_password[pass1]', $clear_value)->save();
            \Drupal::configFactory()->getEditable('miniorange_saml.settings')->set('miniorange_saml_customer_setup_password[pass2]', $clear_value)->save();

            \Drupal::configFactory()->getEditable('miniorange_saml.settings')->set('miniorange_saml_status', $current_status)->save();
            drupal_set_message(t('Your Account Has Been Removed Successfuly!'),'status');
